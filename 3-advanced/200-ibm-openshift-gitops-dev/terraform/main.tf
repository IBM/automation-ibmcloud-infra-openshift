...
module "dev_cluster" {
  ...
  cos_id = var.dev_cluster_cos_id
  disable_public_endpoint = var.dev_cluster_disable_public_endpoint
  exists = var.dev_cluster_exists
  flavor = var.dev_cluster_flavor
  force_delete_storage = var.dev_cluster_force_delete_storage
  ibmcloud_api_key = var.ibmcloud_api_key
  kms_enabled = var.dev_cluster_kms_enabled
  kms_id = var.dev_cluster_kms_id
  kms_key_id = var.dev_cluster_kms_key_id
  kms_private_endpoint = var.dev_cluster_kms_private_endpoint
  login = var.dev_cluster_login
  name = var.dev_cluster_name
  name_prefix = var.name_prefix
  ocp_entitlement = var.dev_cluster_ocp_entitlement
  ocp_version = var.ocp_version
  region = var.region
  resource_group_name = module.dev_resource_group.name
  sync = module.dev_resource_group.sync
  tags = var.dev_cluster_tags == null ? null : jsondecode(var.dev_cluster_tags)
  vpc_name = var.dev_cluster_vpc_name
  vpc_subnet_count = var.dev_cluster_vpc_subnet_count
  vpc_subnets = var.dev_cluster_vpc_subnets == null ? null : jsondecode(var.dev_cluster_vpc_subnets)
  worker_count = var.worker_count
}
module "dev_config" {
  source = "github.com/cloud-native-toolkit/terraform-gitops-cluster-config?ref=v1.0.0"

  banner_background_color = var.dev_config_banner_background_color
  banner_text = var.dev_config_banner_text
  banner_text_color = var.dev_config_banner_text_color
...
module "gitops_repo" {
  ...
  server_name = var.gitops_repo_dev_server_name
